...
def generate_answer():
    ...
    input_tokens = (
        len(openai_tokenizer.encode(prompt))
    ...
    try:
        ...
        if selected_model == "meta_ai":
            ...
            response = ai_client.generate_content(
                prompt, generation_config={"temperature": DEFAULT_TEMPERATURE}
            )
        ...
        # Count output tokens
        output_tokens = (
            len(openai_tokenizer.encode(raw_answer))
            if selected_model == "openai"
            else len(raw_answer.split())
        )
        ...
        metrics = {
            ...
            "temperature": (
                DEFAULT_TEMPERATURE
                if selected_model in ["gemini", "openai", "grok"]
                else "N/A"
            ),
        ...
        return jsonify({"answer": formatted_answer, "metrics": metrics})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
